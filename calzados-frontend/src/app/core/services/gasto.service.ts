...
import { Gasto, GastoRequest, PageResponse, ResumenGastosMes } from '../models/index';
...
export class GastoService {
  ...
  listarMes(year: number, month: number, page = 0, size = 25): Observable<PageResponse<Gasto>> {
    const params = new HttpParams().set('year', year.toString()).set('month', month.toString()).set('page', page).set('size', size);
    return this.http.get<ApiResponse<PageResponse<Gasto>>>(this.api, { params }).pipe(map(r => r.data));
  }
