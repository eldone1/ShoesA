...
public interface VentaRepository extends JpaRepository<Venta, Long> {
    ...
    @Query("""
                ...
                WHERE v.fecha BETWEEN :inicio AND :fin
            """)
    BigDecimal totalVendido(@Param("inicio") LocalDateTime inicio,
            @Param("fin") LocalDateTime fin);

    @Query("""
                SELECT COALESCE(SUM(COALESCE(dv.costoUnitario, 0) * dv.cantidad), 0)
                FROM DetalleVenta dv
                WHERE dv.venta.fecha BETWEEN :inicio AND :fin
            """)
    BigDecimal totalCosto(@Param("inicio") LocalDateTime inicio,
            @Param("fin") LocalDateTime fin);
}
